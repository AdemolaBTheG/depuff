...
export const DraggableSlider: React.FC<DraggableSliderProps> = ({
  ...
  useEffect(() => {
    const safeProgress = Math.min(1, Math.max(0, initialProgress));
    const baseSpacePerLine =
      typeof spacePerLineProp === 'number' ? spacePerLineProp : spacePerLineProp.value;
    const baseProgressWidth = Math.round(linesAmount * baseSpacePerLine);
    const initialOffset = ScreenWidth / 2 - baseProgressWidth * safeProgress;

    scrollContext.value = initialOffset;
    scrollOffset.value = initialOffset;
    clampedScrollOffset.value = initialOffset;
  }, [initialProgress, linesAmount, spacePerLineProp, clampedScrollOffset, scrollContext, scrollOffset]);
